...
def test_cli_run_stability_mode_filters_case_fluctuation_by_min_failure_rate(
    monkeypatch,
    tmp_path: Path,
    capsys,
) -> None:
    ...
    # 3 runs * 3 cases; failure rates: login=1/3, create_vpc=1.0, demo=0.
    run_outputs = iter(
        [
            subprocess.CompletedProcess(
                ["pytest"],
    ...
    assert topn["minFailureRate"] == pytest.approx(0.5)
    assert topn["size"] == 1
    assert topn["byFailureRate"][0]["case"] == "product/create_vpc.yaml"
    assert topn["byCategoryDistribution"][0]["case"] == "product/create_vpc.yaml"


def test_cli_run_stability_mode_filters_case_fluctuation_by_min_planner_failures(
    monkeypatch,
    tmp_path: Path,
    capsys,
) -> None:
    ...
    # 3 runs * 3 cases; plannerFailure totals: login=1, create_vpc=3, demo=0.
    run_outputs = iter(
        [
            subprocess.CompletedProcess(
                ["pytest"],
    ...
    assert topn["minPlannerFailureTotal"] == 2
    assert topn["size"] == 1
    assert topn["byFailureRate"][0]["case"] == "product/create_vpc.yaml"
    assert topn["byCategoryDistribution"][0]["case"] == "product/create_vpc.yaml"
...
def test_cli_run_stability_rejects_invalid_case_min_failure_rate() -> None:
    with pytest.raises(SystemExit):
        cli.main(
            [
                "--run-stability",
...
def test_cli_run_stability_rejects_invalid_case_min_planner_failures() -> None:
    with pytest.raises(SystemExit):
        cli.main(
            [
                "--run-stability",
